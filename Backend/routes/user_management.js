...
LEFT JOIN 
    claim_table c 
    ON u.userId = c.user_id AND c.claim_type IN ('walker')
LEFT JOIN 
    disabilities d 
    ON u.userId = d.user_id
WHERE 
    d.disability_status = 'physical';`
    );
...
router.post("/register-walker", async (req, res) => {
  ...
  try {
    const [result] = await pool.query(
      `SELECT 
        ...
        LEFT JOIN 
        claim_table c 
        ON u.userId = c.user_id AND c.claim_type IN ('medical')
        LEFT JOIN 
        disabilities d 
        ON u.userId = d.user_id;`
    );
...
router.post("/register-medical", async (req, res) => {
  const { userId, claimType } = req.body;
