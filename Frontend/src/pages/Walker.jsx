...
const Walker = () => {
  ...
  const fetchWalkers = async () => {
    try {
      const response = await axios.get(`${host}/api/user_management/walker`);
      const walkersData = response.data.walkers;

      setWalkers(walkersData);
      setTotalClaimed(walkersData.filter((b) => b.claim_tag === 1).length);
      setTotalUnclaimed(walkersData.filter((b) => b.claim_tag === 0).length);
      ...
      if (result.isConfirmed) {
        try {
          const response = await axios.post(
            `${host}/api/user_management/register-walker`,
            {
              userId,
              claimType: "walker",
            }
  ...
  const filteredWalkers = walkers.filter((user) =>
  ...
  return (
    <div style={styles.tableContainer}>
      <div style={styles.header}>
        <h1 style={styles.title}>Walkers and Wheelchair</h1>
