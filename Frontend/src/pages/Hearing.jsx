...
const Hearing = () => {
  ...
  const fetchHearing = async () => {
    try {
      const response = await axios.get(`${host}/api/user_management/hearing`);
      const hearingData = response.data.hearing;

      setHearing(hearingData);
      setTotalClaimed(hearingData.filter((b) => b.claim_tag === 1).length);
      setTotalUnclaimed(hearingData.filter((b) => b.claim_tag === 0).length);
      ...
      if (result.isConfirmed) {
        try {
          const response = await axios.post(
            `${host}/api/user_management/register-hearing`,
            {
              userId,
              claimType: "hearing",
            }
  ...
  const filteredHearing = hearing.filter((user) =>
    user.first_name.toLowerCase().includes(searchQuery.toLowerCase())
  );
  return (
    <div style={styles.tableContainer}>
      <div style={styles.header}>
        <h1 style={styles.title}>Hearing Aid</h1>
